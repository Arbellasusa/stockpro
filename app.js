...
async function seedFirestore(){
  if(!S.fbReady)return;
  showToast('Cargando inventario inicial de Hyde House...');
...
function exportJSON(){
  const data={exportedAt:new Date().toISOString(),project:'hk-pro-housekeeping',hotel:'Hyde House Hotel',exportedBy:S.user?.email,totalItems:S.items.length,totalValue:S.items.reduce((s,i)=>s+(i.qty||0)*(i.price||0),0).toFixed(2),items:S.items};
